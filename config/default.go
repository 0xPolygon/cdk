...
# ------------------------------------------------------------------------------
# These values are used to configure the L1 network
# ------------------------------------------------------------------------------
[L1Config]
  # ----------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# These values are used to configure the L2 network
# ------------------------------------------------------------------------------
[L2Config]
  # ----------------------------------------------------------------------------
...
[Etherman]
  # ----------------------------------------------------------------------------
  # URL is the URL of the L1 RPC node, usually an Ethereum node or an RPC provider
  # ----------------------------------------------------------------------------
  URL="{{L1URL}}"
...
[SequenceSender]
  ...
  # ----------------------------------------------------------------------------
  # PrivateKey is the private key of the sequence-sender
  # ----------------------------------------------------------------------------
  ...
  [Aggregator.EthTxManager]
    ...
    [Aggregator.Synchronizer]
      ...
      [Aggregator.Synchronizer.SQLDB]
        # ----------------------------------------------------------------------------
        # DriverName of the SQL database
        # ----------------------------------------------------------------------------
        DriverName = "sqlite3"
        
        # ----------------------------------------------------------------------------
        # DataSource of the SQL database
        # ----------------------------------------------------------------------------
      ...
      [Aggregator.Synchronizer.Synchronizer]
        ...
        # ----------------------------------------------------------------------------
        # SyncChunkSize is the chunk size to sync
        # ----------------------------------------------------------------------------
        SyncChunkSize = 1000
        ...
        # ----------------------------------------------------------------------------
        # SyncUpToBlock is the block to sync up to
        # ----------------------------------------------------------------------------
        SyncUpToBlock = "finalized"
        
        # ----------------------------------------------------------------------------
        # BlockFinality is the finality of the block
        # ----------------------------------------------------------------------------
      ...
      [Aggregator.Synchronizer.Etherman]
        # ----------------------------------------------------------------------------
        # L1URL is the URL of the L1
        # ----------------------------------------------------------------------------
        L1URL = "{{L1URL}}"
        
        # ----------------------------------------------------------------------------
        # ForkIDChunkSize is the chunk size of the fork ID
        # ----------------------------------------------------------------------------
        ForkIDChunkSize = 100
        ...
        # ----------------------------------------------------------------------------
        # PararellBlockRequest is a flag to enable parallel block requests
        # ----------------------------------------------------------------------------
        ...
        [Aggregator.Synchronizer.Etherman.Validium]
          # ----------------------------------------------------------------------------
          # Enabled is a flag to enable Validium
          # ----------------------------------------------------------------------------
          Enabled = {{IsValidiumMode}}
          
          # ----------------------------------------------------------------------------
          # TrustedSequencerURL is the URL of the trusted sequencer
          # ----------------------------------------------------------------------------
          TrustedSequencerURL = ""
          
          # ----------------------------------------------------------------------------
          # RetryOnDACErrorInterval is the interval to retry on DAC error
          # ----------------------------------------------------------------------------
          RetryOnDACErrorInterval = "1m"
          
          # ----------------------------------------------------------------------------
          # DataSourcePriority is the priority of the data source
          # ----------------------------------------------------------------------------
          DataSourcePriority = ["trusted", "external"]
          
          [Aggregator.Synchronizer.Etherman.Validium.Translator]
            # ----------------------------------------------------------------------------
            # FullMatchRules are the full match rules
            # ----------------------------------------------------------------------------
            FullMatchRules = []
            
          [Aggregator.Synchronizer.Etherman.Validium.RateLimit]
            # ----------------------------------------------------------------------------
            # NumRequests is the number of requests
            # ----------------------------------------------------------------------------
            NumRequests = 1000
            
            # ----------------------------------------------------------------------------
            # Interval is the interval of the rate limit
            # ----------------------------------------------------------------------------
            Interval = "1s"

# ------------------------------------------------------------------------------
# ReorgDetectorL1 configuration
# ------------------------------------------------------------------------------
[ReorgDetectorL1]
  # ----------------------------------------------------------------------------
  ...
  [AggOracle.EVMSender]
    ...
    [AggOracle.EVMSender.EthTxManager]
      ...
      [AggOracle.EVMSender.EthTxManager.Etherman]
        # ----------------------------------------------------------------------------
        # URL is the URL of the L2
        # ----------------------------------------------------------------------------
        URL = "{{L2URL}}"
        
        # ----------------------------------------------------------------------------
        # MultiGasProvider is a flag to enable multiple gas providers
        # ----------------------------------------------------------------------------
        MultiGasProvider = false
...
[LastGERSync]
  ...
  # ----------------------------------------------------------------------------
  # DownloadBufferSize buffer of events to be processed. When the buffer limit is reached,
  # downloading will stop until the processing catches up.
  # ----------------------------------------------------------------------------
  DownloadBufferSize = 100

# ------------------------------------------------------------------------------
# NetworkConfig.L1 configuration
# ------------------------------------------------------------------------------
[NetworkConfig.L1]
  # ----------------------------------------------------------------------------
  # L1ChainID is the chain id of the L1 network
