...
func (p *Prover) WaitRecursiveProof(ctx context.Context, proofID string) (string, common.Hash, common.Hash, error) {
	...
	accInputhHash, err := GetAccInputHashFromProof(p.logger, resProof.RecursiveProof)
	if err != nil && accInputhHash != (common.Hash{}) {
	...
	return resProof.RecursiveProof, sr, accInputhHash, nil
}
...
// GetAccInputHashFromProof returns the account input hash from the proof.
func GetAccInputHashFromProof(logger *log.Logger, proof string) (common.Hash, error) {
	// Log received proof
	logger.Debugf("Received proof to get AccInputHash from: %s", proof)
	...
	for i := accInputHashStartIndex; i < accInputHashFinalIndex; i++ {
		u64, err := strconv.ParseInt(publics.Publics[i], 10, 64)
		if err != nil {
			logger.Fatal(err)
		}
	...
	bigAccInputhHash := fea2scalar(v[:])
	hexAccInputhHash := fmt.Sprintf("%x", bigAccInputhHash)
	if len(hexAccInputhHash)%2 != 0 {
		hexAccInputhHash = "0" + hexAccInputhHash
	}

	return common.HexToHash(hexAccInputhHash), nil
}
