...
import (
	agglayer "github.com/0xPolygon/cdk/aggregator/agglayer"
...
func (_m *AgglayerClientInterfaceMock) SendTx(signedTx agglayer.SignedTx) (common.Hash, error) {
	...
	var r0 common.Hash
	var r1 error
	if rf, ok := ret.Get(0).(func(agglayer.SignedTx) (common.Hash, error)); ok {
		return rf(signedTx)
	}
	if rf, ok := ret.Get(0).(func(agglayer.SignedTx) common.Hash); ok {
		r0 = rf(signedTx)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(agglayer.SignedTx) error); ok {
		r1 = rf(signedTx)
	} else {
		r1 = ret.Error(1)
	}
